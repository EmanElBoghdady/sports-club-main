...
function Filter() {
  ...
  // ── Resolve team display names from /teams (team-analytics only has
  //    a numeric teamId — without this lookup the bars/radar would just
  //    say "Team #1" which is unhelpful).
  ...
  // The Pie component counts items by their flat `sportType` field, but
  // the Team entity exposes `sport` as a nested object (sport.sportType,
  // sport.name). Without this flattening, every team falls into one
  // "Unknown" bucket and the pie shows a single slice.
  const teamsForPie = useMemo(() => {
    return teams.map((t) => ({
      sportType: sportLabel(t?.sport?.sportType || t?.sport?.name || t?.sportType),
    }));
  }, [teams]);
  ...
  const teamBarData = useMemo(() => {
    return teamAnalytics
      .map((t) => ({
        team: teamNameById.get(Number(t.teamId)) || `Team #${t.teamId ?? "?"}`,
  ...
  const radarData = useMemo(() => {
    ...
    return {
      axes,
      teamA: teamNameById.get(Number(a.teamId)) || `Team #${a.teamId ?? "?"}`,
      teamB: b ? (teamNameById.get(Number(b.teamId)) || `Team #${b.teamId ?? "?"}`) : null,
    };
  }, [teamAnalytics, teamNameById]);
                  ...
                  return (
                    <div key={p.id ?? i} className="bg-slate-950/50 border border-slate-900 hover:border-emerald-500/30 rounded-2xl p-6 transition-all group flex justify-between items-center">
                      <div className="flex items-center gap-5">
                        <span className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-emerald-500 font-black text-sm group-hover:bg-emerald-500/10 group-hover:border-emerald-500/30 transition-all">
                          {i + 1}
                        </span>
                        <div>
                          <h4 className="text-sm font-black text-slate-100 uppercase tracking-tight group-hover:text-emerald-400 transition-colors font-mono">
                            {p.playerKeycloakId ? `${p.playerKeycloakId.slice(0, 8)}…` : "Unknown"}
                          </h4>
                          <p className="text-[9px] font-black uppercase tracking-widest mt-1" style={{ color: accent }}>
                            {sport}
                          </p>
                        </div>
                      </div>
