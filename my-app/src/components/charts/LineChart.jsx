...
const LineChart = ({ data }) => {
  ...
  data.forEach(item => {
    ...
    grouped[sport].push(item.averageRating || 0);
  });

  const datasets = Object.keys(grouped).map((sport, i) => ({
    label: sport,
    data: grouped[sport],
    borderColor: ['#3B82F6','#10B981','#8B5CF6'][i % 3],
    tension: 0.4
  }));
  ...
  return (
    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
      <h3 className="text-xs text-slate-400 mb-4">Performance Overview</h3>
      <div className="h-48">
        <Line data={chartData} />
      </div>
    </div>
